document.addEventListener('DOMContentLoaded', () => {
  ...
  productSections.forEach((section) => {
    ...
    // Ensure we only update the cards that are within the visible range
  ...
  slideshowButtons.forEach((button) => {
    button.addEventListener('click', () => {
      ...
      // Reset the active class and margin
      for (let i = 0; i < productCards.length; i++) {
        productCards[i].classList.remove('active');
        productCards[i].style.marginRight = '0px';
      }

      // Activate the new set of product cards
      for (let i = 0; i < productCardsPerRow; i++) {
  ...
  // Initialize lightSlider (ensure this is called only once)
